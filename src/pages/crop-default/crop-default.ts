...
export class CropDefaultPage {
	...
	pictureCrop = this.navParams.get('base64');
	...
	draw() {
		let image = cv.imread('imgCropEdit');
		let gray = new cv.Mat();
		let edge = new cv.Mat();
	...
	crop() {
		...
		cv.imshow('canvasOutputCropEdit', finalDest);
		let canvas = document.getElementById('canvasOutputCropEdit') as HTMLCanvasElement;
		this.pictureCrop =  canvas.toDataURL();			
		image.delete(); gray.delete(); edge.delete(); M.delete(); contours.delete(); hierarchy.delete(); 
