...
export class ImageDocumentPage {
    ...
    saveImage(){
        this.platform.ready().then(() => {
            let canvas = document.getElementById('canvasOutputDoc') as HTMLCanvasElement;
            ...
            if (this.data != null) { 
                ...
                let folderPath = this.file.externalRootDirectory + 'IonScan' + '/' + 'Tài liệu' + '.' + namePhone;
                this.sqlite.create({
                    name: nameDB,
                    location: 'default'
                }).then((db: SQLiteObject) => {                
    ...
    rotateRight() {
        ...
        cv.imshow('canvasOutputDoc', src);
        let canvasOutput = document.getElementById('canvasOutputDoc') as HTMLCanvasElement;
        let picture = document.getElementById("imgDoc") as HTMLImageElement;       
    ...
    rotateLeft() {
        ...
        cv.warpAffine(src, src, M, dsize, cv.INTER_LINEAR, cv.BORDER_CONSTANT, new cv.Scalar());
        cv.imshow('canvasOutputDoc', src);
        var canvasOutput = document.getElementById('canvasOutputDoc') as HTMLCanvasElement;
        let picture = document.getElementById("imgDoc") as HTMLImageElement;
        picture.src = canvasOutput.toDataURL();
        src.delete(); M.delete();
    } 
