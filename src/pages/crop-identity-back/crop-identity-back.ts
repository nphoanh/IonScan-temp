...
imagename = this.navParams.get("imagename");
	...
	draw() {
		let image = cv.imread('imgCropBack');
		let gray = new cv.Mat();
		let edge = new cv.Mat();
		...
		if (approx.rows == 4) {
			...
			cv.imshow('canvasOutputCropBack', image);
		}
		else{
			cv.imshow('canvasOutputCropBack', image);
			return;
	...
	crop() {
		...
		cv.imshow('canvasOutputCropBack', finalDest);
		let canvas = document.getElementById('canvasOutputCropBack') as HTMLCanvasElement;
